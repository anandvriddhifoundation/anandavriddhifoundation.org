      ...
      <div className="container mx-auto px-4">
        <div className="text-center max-w-3xl mx-auto mb-16">
          ...
          <p className="text-lg text-muted-foreground">
            Our social outreach spans three vital segments, each united by one goal — spreading joy and
            knowledge through immersive virtual experiences.
          </p>
        </div>
          ...
          {pillars.map((pillar, i) => (
            ...
            >
              ...
              <div className="lg:w-1/2">
                ...
                <p className="text-muted-foreground text-lg leading-relaxed mb-6">{pillar.description}</p>
                {pillar.points && (
                  <ul className="space-y-3">
                    {pillar.points.map((point, idx) => (
                      <li key={idx} className="flex items-start text-muted-foreground text-[1.05rem]">
                        <span className="text-amber-500 mr-3 text-xl leading-none">•</span>
                        <span>{point}</span>
                      </li>
                    ))}
                  </ul>
